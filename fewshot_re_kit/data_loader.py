...
class JSONFileDataLoader(FileDataLoader):
    def _load_preprocessed_file(self):
        ...
        if not os.path.exists(word_npy_file_name) or \
                not os.path.exists(head_npy_file_name) or \
                not os.path.exists(tail_npy_file_name) or \
    ...
    def __init__(self, file_name, word_vec_file_name, max_length=40, entity_length=6, case_sensitive=False,
                 reprocess=False, cuda=True):
        ...
        if reprocess or not self._load_preprocessed_file():  # Try to load pre-processed files:
            ...
            self.data_head = np.zeros((self.instance_tot, self.entity_length), dtype=np.int32)
            self.data_tail = np.zeros((self.instance_tot, self.entity_length), dtype=np.int32)
            ...
            for relation in self.ori_data:
                ...
                for ins in self.ori_data[relation]:
                    ...
                    # head
                    cur_ref_data_head = self.data_head[i]
                    for j, word in enumerate(head):
                        if j < entity_length:
                            if word in self.word2id:
                                cur_ref_data_head[j] = self.word2id[word]
                            else:
                                cur_ref_data_head[j] = UNK
                    for j in range(j + 1, entity_length):
                        cur_ref_data_head[j] = BLANK

                    # tail
                    cur_ref_data_tail = self.data_tail[i]
                    for j, word in enumerate(tail):
                        if j < entity_length:
                            if word in self.word2id:
                                cur_ref_data_tail[j] = self.word2id[word]
                            else:
                                cur_ref_data_tail[j] = UNK
                    for j in range(j + 1, entity_length):
                        cur_ref_data_tail[j] = BLANK
    ...
    def lookup(self, ins):
        ...
        # head
        cur_ref_data_head = np.zeros(self.entity_length, dtype=np.int32)
        for j, word in enumerate(head.split(' ')):
            if j < self.entity_length:
                if word in self.word2id:
                    cur_ref_data_head[j] = self.word2id[word]
                else:
                    cur_ref_data_head[j] = self.word2id['UNK']
        for j in range(j + 1, self.entity_length):
            cur_ref_data_head[j] = self.word2id['BLANK']

        # tail
        cur_ref_data_tail = np.zeros(self.entity_length, dtype=np.int32)
        for j, word in enumerate(tail.split(' ')):
            if j < self.entity_length:
                if word in self.word2id:
                    cur_ref_data_tail[j] = self.word2id[word]
                else:
                    cur_ref_data_tail[j] = self.word2id['UNK']
        for j in range(j + 1, self.entity_length):
            cur_ref_data_tail[j] = self.word2id['BLANK']
        return cur_ref_data_word, cur_ref_data_head, cur_ref_data_tail, data_pos1, data_pos2, data_mask, data_length
    ...
    def next_batch(self, B, N, K, Q, noise_rate=0):
        ...
        support['head'] = Variable(torch.from_numpy(np.stack(support['head'], 0)).long().view(-1, self.entity_length))
        support['tail'] = Variable(torch.from_numpy(np.stack(support['tail'], 0)).long().view(-1, self.entity_length))
        ...
        query['head'] = Variable(torch.from_numpy(np.stack(query['head'], 0)).long().view(-1, self.entity_length))
        query['tail'] = Variable(torch.from_numpy(np.stack(query['tail'], 0)).long().view(-1, self.entity_length))
